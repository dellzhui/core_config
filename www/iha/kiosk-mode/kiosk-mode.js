...
),l=n(e("@babel/runtime/helpers/createClass"));function v(e,t){var r="undefined"!=typeof Symbol&&e[Symbol.iterator]||e["@@iterator"];if(!r){if(
...
;this.processConfig(t,e)}catch(e){this.llAttempts<300?setTimeout(function(){return r.getConfig(t)},50):(console.log(
"Lovelace config not found, continuing with default configuration."),this.processConfig(t,{}))}}},{key:"processConfig",value:function(e,t){var r=this,
n=this.ha.hass.panelUrl;window.kioskModeEntities[n]||(window.kioskModeEntities[n]=[]),
...
;var i=this.user.is_admin?t.admin_settings:t.non_admin_settings;if(i){var a,s=v(i);try{for(s.s();!(a=s.n()).done;){var u=a.value;this.setOptions(u)}
}catch(e){s.e(e)}finally{s.f()}}if(t.user_settings){var l,c=v(this.array(t.user_settings));try{for(c.s();!(l=c.n()).done;){var h=l.value;this.array(
h.users).some(function(e){return e.toLowerCase()==r.user.name.toLowerCase()})&&this.setOptions(h)}}catch(e){c.e(e)}finally{c.f()}}
o=this.ignoreMobile?null:t.mobile_settings;o&&(i=o.custom_width||812,window.innerWidth<=i&&this.setOptions(o))
;t=this.ignoreEntity?null:t.entity_settings;if(t){var d,f=v(t);try{for(f.s();!(d=f.n()).done;){var p=d.value,y=Object.keys(p.entity)[0]
;window.kioskModeEntities[n].includes(y)||window.kioskModeEntities[n].push(y),this.ha.hass.states[y].state==p.entity[y]&&("hide_header"in p&&(
this.hideHeader=p.hide_header),"hide_sidebar"in p&&(this.hideSidebar=p.hide_sidebar),"hide_overflow"in p&&(this.hideOverflow=p.hide_overflow),
"kiosk"in p&&(this.hideHeader=this.hideSidebar=p.kiosk))}}catch(e){f.e(e)}finally{f.f()}}this.insertStyles(e)}},{key:"insertStyles",value:function(e){
...
key:"watchDashboards",value:function(e){e.forEach(function(e){var t,r=v(e.addedNodes);try{for(r.s();!(t=r.n()).done;){var n=t.value
...
ver:"%cversion 1.7.0 "},f=Math.max.apply(Math,(0,a.default)(Object.values(d).map(function(e){return e.length}))),p=0,y=Object.entries(d
);p<y.length;p++){var m=(0,i.default)(y[p],1)[0];d[m].length<=f&&(d[m]=d[m].padEnd(f)),"header"==m&&(d[m]="".concat(d[m].slice(0,-1),"⋮ "))}
